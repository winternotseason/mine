...
const PostForm = () => {
    ...
    onSuccess: async (insertedPost: IPost) => {
      ...
      if (queryClient.getQueryData(["categories"])) {
        queryClient.setQueryData(["categories"], (prevData: Category[]) => {
          ...
          console.log("업데이트 데이터, 카테고리 ", shallow);
          return shallow;
        });
      }
      // 지역 업데이트
      if (queryClient.getQueryData(["region"])) {
        queryClient.setQueryData(["region"], (prevData: Region[]) => {
          ...
          console.log(queryClient.getQueryData(["region"]))
          console.log("업데이트 데이터, 지역 ", shallow);
          return shallow;
        });
      }
    },
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-14 pb-10">
      {/* 이미지 업로드 */}
